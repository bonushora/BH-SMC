...
app.post('/api/voluntarios', async (req, res) => {
    try {
        ...
        res.json({ success: true, voluntario, mensagem: "Voluntário cadastrado com 10 horas iniciais!" });
    } catch (error) {
        res.status(400).json({ error: "Número já cadastrado no sistema." });
    }
});

// 3. Motor Central de Transações
app.post('/api/transacoes', async (req, res) => {
    try {
        const { numero, horas, tipo, descricao } = req.body; 

        const voluntario = await prisma.voluntario.findUnique({ where: { numero } });
        ...
        const [transacao, voluntarioAtualizado] = await prisma.$transaction([
            prisma.transacao.create({
                data: { voluntarioId: voluntario.id, horas, tipo, descricao }
            }),
        ...
        res.status(500).json({ error: "Erro ao processar transação no Ledger." });
    }
});
...
app.post('/api/chat/sovereign', async (req, res) => {
    ...
    try {
        ...
        const voluntario = await prisma.voluntario.findUnique({ where: { numero: whatsapp } });
        if (!voluntario) {
            return res.json({ response: "Bem-vindo ao Ledger SECIS! Você ainda não está cadastrado.", opcoes: ["Cadastrar Agora"] });
        }
        return res.json({ response: `Seu saldo atual é de ${voluntario.saldo} bônus-horas.`, opcoes: ["Ver Benefícios Disponíveis", "Voltar ao Menu"] });
    } catch (error) {
...
app.post('/api/homologacao/resgate', async (req, res) => {
  ...
  if (!whatsapp || !tipoBeneficio) {
    return res.status(400).json({ success: false, error: "Parâmetros 'whatsapp' e 'tipoBeneficio' são obrigatórios." });
  }

  try {
    const voluntario = await prisma.voluntario.findUnique({ where: { numero: whatsapp } });
    if (!voluntario) return res.status(404).json({ success: false, error: "Voluntário não encontrado." });

    const beneficio = await prisma.beneficio.findFirst({ where: { tipo: tipoBeneficio } });
    if (!beneficio) return res.status(404).json({ success: false, error: "Benefício não homologado." });

    if (voluntario.saldo < beneficio.custoHoras) {
      return res.status(400).json({ success: false, error: `Saldo insuficiente. Necessário: ${beneficio.custoHoras}h, Disponível: ${voluntario.saldo}h.` });
    }
    ...
    const [transacao, voluntarioAtualizado] = await prisma.$transaction([
      ...
      prisma.voluntario.update({
        where: { id: voluntario.id },
        data: { saldo: { decrement: beneficio.custoHoras } }
      })
    ]);
    ...
    return res.status(200).json({ success: true, comprovante });
  } catch (error) {
